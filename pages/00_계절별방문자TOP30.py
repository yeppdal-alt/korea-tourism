...
# -----------------------------------------------------------
# 연중(봄+여름+가을+겨울을 모두 합친) 방문자 많은 지역 TOP30
# -----------------------------------------------------------
st.subheader(f"📅 연중 방문자 많은 지역 TOP{TOP_N} (전체 계절 합산)")
st.caption("2025년 한 해 전체(봄~겨울 합산) 기준으로, 외지인+외국인 방문객수가 가장 많았던 지역입니다.")

annual_total = df.groupby("지역명", as_index=False)["touNum"].sum()
annual_top_n = annual_total.sort_values("touNum", ascending=False).head(TOP_N)

if annual_top_n.empty:
    st.info("연간 데이터를 찾을 수 없습니다.")
else:
    # 가로 막대 그래프는 아래에서 위로 그려지므로, 1위가 맨 위에 오도록 오름차순으로 정렬해서 넣습니다.
    annual_top_n = annual_top_n.sort_values("touNum")
    ...
    fig_annual = px.bar(
        ...
        color_continuous_scale="Plasma",  # 계절별 그래프와 구분되는 전용 색감
        labels={"touNum": "방문객수(명)", "지역명": "시/군/구"},
        title=f"연중 외지인·외국인 방문자 TOP{TOP_N}",
    )
