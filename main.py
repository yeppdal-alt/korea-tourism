...
#   0) 계절별 방문객수 (DataLabService /locgoRegnVisitrDDList)
#      - 시/도만 선택하면: 버튼 없이 자동으로 소속 시/군/구별 비교 그래프를 보여줌
#      - 시/군/구까지 선택하면: 버튼 없이 자동으로 그 지역의 계절별 그래프+표를 보여줌
#   1) 숙박정보      (/searchStay2)
#   2) 공통정보      (/detailCommon2)
#   3) 개요정보      (/detailCommon2 의 overview 항목)
#   4) 반려동물 동반 여행정보 (/detailPetTour2)
#   5) 선택한 지역 위치를 대한민국 지도 위에 표시 (화면 맨 아래, 작게, 한글 지명 표시)
#
...
# 지도/탭 등 아래에서 계속 쓸 "선택한 지역 이름"을 미리 계산해둡니다.
# (실제 지도 표시는 화면 맨 아래로 옮겼습니다)
if selected_sigungu_name != "전체":
...
# ---------------------------------------------------------
# 5. 계절별 방문객수 (DataLabService /locgoRegnVisitrDDList)
#    - 시/도만 선택("전체"): 버튼 없이 자동으로 소속 시/군/구별 비교 그래프를 보여줍니다.
#    - 시/군/구까지 선택: 버튼 없이 자동으로 그 지역의 계절별 그래프+표를 보여줍니다.
#    - 연도는 2025년으로 고정합니다.
# ---------------------------------------------------------
VISITOR_CHART_YEAR = 2025  # 방문객수 데이터는 2025년만 조회합니다.
...
if not TOURNUM_API_KEY:
    ...
    # 기초 지자체(시/군/구) 데이터는 지역 필터가 없는 API라 전국 데이터를 한 번에 받아온 뒤
    # 화면에서 필요한 시/도만 골라 씁니다. 24시간 캐시되어 있어 같은 날 다시 열어도 빠릅니다.
    with st.spinner("방문객수 데이터를 불러오는 중입니다... (최초 조회 시 전국 데이터를 받아오느라 시간이 걸릴 수 있어요)"):
    ...
    if not yearly_items:
        ...
        sido_df = (
            df[df["signguCode"].astype(str).str.startswith(sido_prefix)].copy()
            if sido_prefix
        ...
        if sido_df.empty:
            ...
            sido_df["touNum"] = pd.to_numeric(sido_df["touNum"], errors="coerce")
            ...
            if not is_sigungu_view:
                ...
                # -------- 시/군/구까지 선택: 그 지역의 계절별 상세 --------
                region_df = sido_df[sido_df["signguNm"] == selected_sigungu_name].copy()
                if region_df.empty:
                    # 이름 표기가 살짝 다를 수 있어(예: 공백 등) 부분일치로 한 번 더 시도합니다.
                    region_df = sido_df[
                        sido_df["signguNm"].astype(str).str.contains(selected_sigungu_name, na=False)
                    ].copy()
...
# ---------------------------------------------------------
# 7. 선택한 지역 위치를 대한민국 지도 위에 표시 (화면 맨 아래)
# ---------------------------------------------------------
